...
MAX_WORD_COUNT_LIMIT = 80 #Prev 80
BATCH_SIZE = 16
TRAIN_BATCH_SIZE = 8
    ...
    def get_next_conversation_prompt_from_conversation(self, conversation, text_description, initial_prompt=False):
        ...
        if initial_prompt:
            current_text = {
                "role": "user",
                "content": [
                    {"type": "text", "text": f"Use this prompt to generate a detailed description of a video: {text_description}"},
                ],
            }
        else:
            current_text = {
                "role": "user",
                "content": [
                    {"type": "text", "text": f"Use this prompt and the context of conversation before to generate a detailed description of next scene of the video: {text_description}"},
                ],
            }
        conversation.append(current_text)
...
def load_model(model_repo_id,model_file_name):
    ...
    temporal_int_gen_model = InteractiveChatVideoGenModel(
    batch_size=TRAIN_BATCH_SIZE, use_llm=False, use_tpu_vm=False, use_xla=False,use_vae=True)

    temporal_int_gen_model.configure_model()

    temporal_int_gen_model.load_state_dict(state_dict)

    #print(temporal_int_gen_model.latent_diffusion_model.alphas_cumprod)

    temporal_int_gen_model.latent_diffusion_model.register_schedule(given_betas=None, beta_schedule="linear", timesteps=temporal_int_gen_model.latent_diffusion_model_config['params']['timesteps'],linear_start=temporal_int_gen_model.latent_diffusion_model_config['params']['linear_start'], linear_end=temporal_int_gen_model.latent_diffusion_model_config['params']['linear_end'], cosine_s=8e-3)
    ...
    temporal_int_gen_model.multimodal_llm=LlavaNextVideoForConditionalGeneration.from_pretrained(temporal_int_gen_model.multimodal_llm_id,torch_dtype=DTYPE_PT,load_in_4bit=True,device_map="cuda:0")

    temporal_int_gen_model.multimodal_llm_processor = LlavaNextVideoProcessor.from_pretrained(temporal_int_gen_model.multimodal_llm_id)

    temporal_int_gen_model.latent_diffusion_model=temporal_int_gen_model.latent_diffusion_model.to("cuda")
    
    temporal_int_gen_model.bridge_mlp=temporal_int_gen_model.bridge_mlp.to("cuda")

    temporal_int_gen_model.vae=temporal_int_gen_model.vae.to("cuda")

temporal_int_gen_model = None
@torch.no_grad()
def run_temporal_decoder_inference(prompt,model_repo_id,model_file_name,num_inference_steps=50,fps=24,ddim_eta=1.0,unconditional_guidance_scale=1.0):
    global temporal_int_gen_model
    
    if temporal_int_gen_model is None:
        print("First run, loading model...")
        load_model(model_repo_id,model_file_name)
    
    b_text_description=[prompt]

    b_llm_text_input = [temporal_int_gen_model.get_next_conversation_prompt_from_conversation([],text_desc,initial_prompt=True) for text_desc in b_text_description]

    b_llm_input_processed = temporal_int_gen_model.multimodal_llm_processor([elem[1] for elem in b_llm_text_input],padding=True, return_tensors="pt")

    b_llm_output = temporal_int_gen_model.multimodal_llm.generate(b_llm_input_processed['input_ids'],return_dict_in_generate=True,output_hidden_states=True, max_new_tokens=MAX_WORD_COUNT_LIMIT, do_sample=False)

    b_llm_hidden_states =  temporal_int_gen_model.get_last_layer_hidden_states_from_llm_output_batched_torch(b_llm_output)[0]
    
    b_llm_hidden_states=b_llm_hidden_states.flatten(0,1)

    b_llm_hidden_states=b_llm_hidden_states.to("cuda")

    bridge_net_op=temporal_int_gen_model.bridge_mlp(b_llm_hidden_states)
    ...
    temporal_int_gen_model.batch_size=1

    temporal_int_gen_model.eval()

    noise_shape = [temporal_int_gen_model.batch_size, temporal_int_gen_model.latent_diffusion_model.channels, temporal_int_gen_model.num_frames, temporal_int_gen_model.latent_size, temporal_int_gen_model.latent_size]
        
    cond = {"c_crossattn": [bridge_net_op], "fps": fps}

    batch_samples = batch_ddim_sampling(temporal_int_gen_model, cond, noise_shape, 1, \
...
if __name__=="__main__":
    ...
    model_file_name="pytorch_model_unet_cross_attn_192_temp_decoder_ddim_optical_flow_eph_8_sakuga.bin"
    num_inference_steps=500
    fps=24
    ddim_eta=1.0
    unconditional_guidance_scale=8.0

    run_temporal_decoder_inference(text_prompt,model_repo_id,model_file_name,num_inference_steps,fps,ddim_eta,unconditional_guidance_scale)
